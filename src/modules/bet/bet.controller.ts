  ...
  async voteBet(@Body() voteDto: VoteDto) {
    ...
    try {
      ...
      if (new Date(bet.endTime) < currentTime) {
        throw new HttpException('Bet has already closed', HttpStatus.BAD_REQUEST);
      }
      ...
      if (userWallet.balance < bet.minAmount) {
        throw new HttpException('Insufficient balance to place bet', HttpStatus.BAD_REQUEST);
      }

      // 5. Check if option is valid
      if (!bet.options.includes(votedOption)) {
        throw new HttpException('Invalid voting option', HttpStatus.BAD_REQUEST);
      }
